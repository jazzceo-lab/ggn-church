...
function DailyVersePushSettings() {
  ...
  return (
    <div className="mb-6 space-y-3 rounded-xl border border-black/10 bg-white/60 p-4 dark:border-white/10 dark:bg-white/5">
      ...
      <p className="text-xs text-foreground/50">
        회원정보에서 &ldquo;오늘의 성경 알림&rdquo;을 켠 회원에게 이미지카드 푸시를 보내요. 위 스위치를
        끄면 시간/주기 설정과 무관하게 전체 발송이 멈춰요.
      </p>

      <div className={`space-y-3 ${settings.enabled ? "" : "pointer-events-none opacity-40"}`}>
